...
void CUserBoard::Render()
{
	...
	for (int i = 1; i < 7; i++) {
		if (i <= fly_mark) {
			animations->Get(ID_ANI_ARROW_NORMAL)->Render(x - 78 + 53 + 4 + (i-1) * 8, y - 18 + 8 + 6);
		}
		else
			animations->Get(ID_ANI_ARROW_ACTIVE)->Render(x - 78 + 53 + 4 + (i-1) * 8, y - 18 + 8 + 6);
	}
	if (fly_mark == 6)
		animations->Get(ID_ANI_P_MARK_ACTIVE)->Render(x - 78 + 53 + 6 + 6 * 8 +2 , y - 18 + 8 + 6);
	else
		animations->Get(ID_ANI_P_MARK_NORMAL)->Render(x - 78 + 53 + 6 + 6 * 8 + 2, y - 18 + 8 + 6);
}	
